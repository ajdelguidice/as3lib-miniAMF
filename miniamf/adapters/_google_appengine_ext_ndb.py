...
class NDBClassAlias(gae_base.BaseDatastoreClassAlias):
    ...
    def getDecodableAttributes(self, obj, attrs, codec=None):
        ...
        if self.repeated_properties:
            for name, prop in self.repeated_properties.items():
                ...
                for idx, val in enumerate(value):
                    value[idx] = models.decode_model_property(
                        obj,
                        prop,
                        val
                    )
        ...
        if self.non_repeated_properties:
            models.decode_model_properties(
                obj,
    ...
    def encode_property(self, obj, prop, value):
        if not prop._repeated:
            return models.encode_model_property(
                obj,
                prop,
                value
            )
        ...
        for idx, val in enumerate(value):
            value[idx] = models.encode_model_property(
                obj,
                prop,
                val
            )
...
@models.register_property_decoder(ndb.KeyProperty)
...
@models.register_property_decoder(ndb.DateProperty)
...
@models.register_property_decoder(ndb.FloatProperty)
...
@models.register_property_decoder(ndb.IntegerProperty)
...
@models.register_property_encoder(ndb.KeyProperty)
...
@models.register_property_encoder(ndb.TimeProperty)
def encode_time_property(obj, prop, value):
    ...
    raise miniamf.EncodeError('ndb.TimeProperty is not supported by MiniAMF')


@models.register_property_encoder(ndb.DateProperty)
...
if hasattr(ndb.model, '_NotEqualMixin'):
    not_equal_mixin = miniamf.register_class(ndb.model._NotEqualMixin)
    not_equal_mixin.compile()

    del not_equal_mixin

# initialise the module here: hook into miniamf
miniamf.register_alias_type(NDBClassAlias, ndb.Model, ndb.Expando)
miniamf.add_type(ndb.Query, util.to_list)
miniamf.add_type(ndb.Model, encode_ndb_instance)
miniamf.add_post_decode_processor(post_ndb_process)
miniamf.add_type(ndb.Key, encode_ndb_key)

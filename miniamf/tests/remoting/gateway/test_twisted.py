...
class TwistedServerTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.p = reactor.listenTCP(0, server.Site(root), interface=b"127.0.0.1")
    ...
    def getPage(self, data=None, **kwargs):
        ...
        method = kwargs.get('method', b'POST')
    ...
    def doRequest(self, service, body=missing, type=miniamf.AMF3, raw=False,
                  decode=True):
        if not raw:
        ...
        def decode_response(response):
            # TODO: calling readBody here is deprecated
            body = yield client.readBody(response)
            data = remoting.decode(body)
            return data
    ...
    def test_process_request(self):
        ...
        def cb(response):
            self.assertEqual(response.amfVersion, miniamf.AMF3)
    ...
    def test_expose_request(self):
        ...
        d = self.doRequest('echo', 'hello', type=miniamf.AMF0)
    ...
    def test_timezone(self):
        import datetime
        from miniamf.util import utcnow
...
class TwistedGatewayTestCase(BaseTestCase):
    def test_finalise_request(self):
        ...
        self.assertEqual(request.content, b'xyz')
    ...
    def test_get_processor(self):
        a3 = miniamf.ASObject({'target': 'null'})
        a0 = miniamf.ASObject({'target': 'foo.bar'})

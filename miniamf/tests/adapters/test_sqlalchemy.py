...
class BaseTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        miniamf.register_class(User, 'server.User')
        miniamf.register_class(Address, 'server.Address')
        miniamf.register_class(LazyLoaded, 'server.LazyLoaded')

    def tearDown(self):
        clear_mappers()

        miniamf.unregister_class(User)
        miniamf.unregister_class(Address)
        miniamf.unregister_class(LazyLoaded)
...
class BaseClassAliasTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.alias = miniamf.get_class_alias(User)
...
class ApplyAttributesTestCase(BaseClassAliasTestCase):
    def test_undefined(self):
        ...
        attrs = {
            ...
            'addresses': [],
            'lazy_loaded': [],
            'another_lazy_loaded': miniamf.Undefined,  # <-- the important bit
            'id': None,
            'name': 'test_user'
        }
    ...
    def test_decode_unaliased(self):
        ...
        attrs = {
            ...
            # instance of miniamf.ASObject
            'another_lazy_loaded': [
                miniamf.ASObject({'id': 1, 'user_id': None})
            ],
            'id': None,
            'name': 'test_user'
        }

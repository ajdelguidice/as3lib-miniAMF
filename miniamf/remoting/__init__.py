...
class RemotingError(miniamf.BaseError):
    """
...
miniamf.add_error_class(RemotingCallFailed, ERROR_CODES[ERROR_CALL_FAILED])
...
class Envelope(object):
    ...
    @ivar amfVersion: AMF encoding version. See L{miniamf.ENCODING_TYPES}
...
def _read_header(stream, decoder, strict=False):
    ...
    @type stream: L{BufferedByteStream<miniamf.util.BufferedByteStream>}
    @param decoder: An AMF0 decoder.
    @param strict: Use strict decoding policy. Default is C{False}. Will raise
        a L{miniamf.DecodeError} if the data that was read from the stream does
...
def _write_header(name, header, required, stream, encoder, strict=False):
    ...
    @param stream: L{BufferedByteStream<miniamf.util.BufferedByteStream>} that
...
def _read_body(stream, decoder, strict=False, logger=None):
    ...
    if status == STATUS_ERROR and isinstance(data, miniamf.ASObject):
        data = get_fault(data)
...
def decode(stream, strict=False, logger=None, timezone_offset=None,
    ...
    decoder.use_amf3 = msg.amfVersion == miniamf.AMF3
...
def encode(msg, strict=False, logger=None, timezone_offset=None, **kwargs):
    ...
    @type timezone_offset: U{datetime.datetime.timedelta<http://
        docs.python.org/library/datetime.html#datetime.timedelta>}
    @rtype: L{BufferedByteStream<miniamf.util.BufferedByteStream>}
    """
    ...
    if msg.amfVersion == miniamf.AMF3:
...
def get_exception_from_fault(fault):
    return miniamf.ERROR_CLASS_MAP.get(fault.code, RemotingError)


miniamf.register_class(ErrorFault)
